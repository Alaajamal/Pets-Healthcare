			...
			if procedures:
				...
				if not service_item:
					msg = _(("Please Configure {0} in ").format("Clinical Procedure Consumable Item") \
						+ """<b><a href="#Form/Pets Healthcare Settings">Pets Healthcare Settings</a></b>""")
					frappe.throw(msg)
				for procedure in procedures:
					procedure_obj = frappe.get_doc("Clinical Procedure", procedure['name'])
					item_to_invoice.append({'reference_type': 'Clinical Procedure', 'reference_name': procedure_obj.name,
...
def get_healthcare_service_item(service_item_field):
	return frappe.db.get_value("Pets Healthcare Settings", None, service_item_field)
...
def manage_invoice_submit_cancel(doc, method):
	...
	if method=="on_submit" and frappe.db.get_value("Pets Healthcare Settings", None, "create_test_on_si_submit") == '1':
