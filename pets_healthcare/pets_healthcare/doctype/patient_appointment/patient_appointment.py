...
def appointment_valid_in_fee_validity(appointment, valid_end_date, invoiced, ref_invoice):
	valid_days = frappe.db.get_value("Pets Healthcare Settings", None, "valid_days")
	max_visit = frappe.db.get_value("Pets Healthcare Settings", None, "max_visit")
...
def cancel_sales_invoice(sales_invoice):
	if frappe.db.get_value("Pets Healthcare Settings", None, "manage_appointment_invoice_automatically") == '1':
...
def confirm_sms(doc):
	if frappe.db.get_value("Pets Healthcare Settings", None, "app_con") == '1':
		message = frappe.db.get_value("Pets Healthcare Settings", None, "app_con_msg")
...
def set_appointment_reminder():
	if frappe.db.get_single_value("Pets Healthcare Settings", "app_rem"):
		remind_before = datetime.datetime.strptime(frappe.db.get_single_value("Pets Healthcare Settings", "rem_before"), '%H:%M:%S')
		...
		for appointment in appointment_list:
			...
			message = frappe.db.get_single_value("Pets Healthcare Settings", "app_rem_msg")
			send_message(doc, message)
